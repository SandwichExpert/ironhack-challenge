...
export default function Celebrities() {
    ...
    for(let page = 1; page <=100; page++ ){
      ...
      axios
        ...
        .then(res => {
          ...
          setCelebrities(lastCelebrities=>[...lastCelebrities, ...res.data.results]);
        });
    }
  }, []);
  ...
  let filteredCelebrities = celebrities.filter(celebrity=> celebrity.name.toUpperCase().includes(search.toUpperCase()));
      ...
      <div className="celebsContainer">
        <div className="celebsList">
            ...
            <div className="left_celebs">
              ...
              {filteredCelebrities
              .map(celebrity => (
                  <tr key={celebrity.id} onClick={()=> setSelectedCelebrityId(celebrity.id)}
                  className="definite">
                    <td>
                      <img
                    ...
                    <td className={selectedCelebrityId === celebrity.id ? "active_name" : 'inactive' }>{celebrity.name}</td>
                  </tr>
                ))}
             </tbody>
              </table>
